...
from qfluentwidgets import (setTheme, Theme, PrimaryPushButton, PushButton, ComboBox, LineEdit, MessageBox, BodyLabel, TitleLabel)
...
class InputPage(QWidget):
    def __init__(self, parent=None):
        ...
        # 名单文件输入
        self.student_file_input = self.create_file_input("名单文件:", "选择文件", self.select_student_file)
        input_layout.addWidget(self.student_file_input)

        # 教室文件输入
        self.classroom_file_input = self.create_file_input("教室文件:", "选择文件", self.select_classroom_file)
    ...
    def select_student_file(self):
        filename, _ = QFileDialog.getOpenFileName(
            self, "选择名单文件", "", "Excel files (*.xlsx);;All files (*.*)"
        )
        if filename:
...
class MainWindow(QMainWindow):
    ...
    def show_about(self):
        MessageBox("关于", data.ABOUT_TEXT, self).exec()

    def show_format_help(self):
        MessageBox("格式说明", data.HELP_TEXT, self).exec()
